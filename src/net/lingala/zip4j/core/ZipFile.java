...
public class ZipFile {
	...
	/**
	 * 压缩文件列表，列表中文件既可以是文件又可以是文件夹又可以是混合列表，但是必须在同一个父文件夹下，但这里没做检测
	 * 
	 * @param sourceFileList
	 * @param parameters
	 */
	public void createZipFileFromFiles(ArrayList<File> sourceFileList,
			ZipParameters parameters) {

		try {
			ArrayList<File> fileList = new ArrayList<File>();
			for (int i = 0; i < sourceFileList.size(); i++) {
				File file = sourceFileList.get(i);
				if (file != null) {
					if (file.exists()) {
						if (file.isFile()) {
							fileList.add(file);
						} else {
							fileList.addAll(ZipEngine.getAddFolderToZip(file,
									parameters));
						}
					} else {
						throw new FileNotFoundException("non-file in files");
					}
				} else {
					throw new FileNotFoundException("non-file in files");
				}
			...
			if (fileList.size() > 0) {
				createZipFile(fileList, parameters, false, -1);
			} else {
				throw new FileNotFoundException("non-file in files");
			}
	 ...
	 * 
	 * @param sourceFile
	 *            - File to be added to the zip file
	 * @param parameters
	 ...
	 *            - parameters to create the zip file
	 * @param splitArchive
	 *            - if archive has to be split or not
	 * @param splitLength
	 *            - if archive has to be split, then length in bytes at which it
	 *            has to be split
	 * @throws ZipException
	 */
	public void createZipFile(File sourceFile, ZipParameters parameters,
	 ...
	 *            - File to be added to the zip file
	 * @param parameters
	 *            - zip parameters for this file list
	 * @param splitArchive
	 *            - if archive has to be split or not
	 * @param splitLength
	 *            - if archive has to be split, then length in bytes at which it
	...
	public void createZipFile(ArrayList sourceFileList,
		...
		if (sourceFileList == null) {
			throw new ZipException(
					"input file ArrayList is null, cannot create zip file");
		}

		if (!Zip4jUtil.checkArrayListTypes(sourceFileList,
				InternalZipConstants.LIST_TYPE_FILE)) {
			throw new ZipException(
					"One or more elements in the input ArrayList is not of type File");
		}
	...
	public void createZipFileFromFolder(String folderToAdd,
		...
		if (!Zip4jUtil.isStringNotNullAndNotEmpty(folderToAdd)) {
			throw new ZipException(
					"folderToAdd is empty or null, cannot create Zip File from folder");
		}
	 ...
	 * 
	 * @param sourceFile
	 *            - File to tbe added to the zip file
	 * @param parameters
	...
	public void addFiles(ArrayList sourceFileList, ZipParameters parameters)
		...
		if (sourceFileList == null) {
			throw new ZipException(
					"input file ArrayList is null, cannot add files");
		}
		...
		if (parameters == null) {
			throw new ZipException(
					"input parameters are null, cannot add files to zip");
		}
		...
		if (Zip4jUtil.checkFileExists(file)) {
			if (zipModel.isSplitArchive()) {
				throw new ZipException(
						"Zip file already exists. Zip file format does not allow updating split/spanned files");
			}
		}
	...
	public void addFolder(String path, ZipParameters parameters)
		...
		if (!Zip4jUtil.isStringNotNullAndNotEmpty(path)) {
			throw new ZipException(
					"input path is null or empty, cannot add folder to zip file");
		}
	...
	private void addFolder(File path, ZipParameters parameters,
		...
		ZipEngine zipEngine = new ZipEngine(zipModel);
		zipEngine
				.addFolderToZip(path, parameters, progressMonitor, runInThread);
	 ...
	 * not exist, then this method throws an exception.<br>
	 * <br>
	...
	private void readZipInfo() throws ZipException {
		...
		try {
			raf = new RandomAccessFile(new File(file),
	 ...
	 * @throws ZipException
	 */
	public void extractAll(String destPath, UnzipParameters unzipParameters)
			throws ZipException {
		...
		if (zipModel == null) {
			throw new ZipException(
					"Internal error occurred when extracting zip file");
		}
		...
		unzip.extractAll(unzipParameters, destPath, progressMonitor,
				runInThread);
	...
	/**
	 * Extracts a specific file from the zip file to the destination path. If
	 * destination path is invalid, then this method throws an exception. <br>
	 * <br>
	...
	public void extractFile(FileHeader fileHeader, String destPath,
		...
		fileHeader.extractFile(zipModel, destPath, unzipParameters,
				newFileName, progressMonitor, runInThread);

	}

	/**
	 * Extracts a specific file from the zip file to the destination path. This
	 * method first finds the necessary file header from the input file name. <br>
	 * <br>
	 ...
	 * If newFileName is not null or empty, newly created file name will be
	 * replaced by the value in newFileName. If this value is null, then the
	 * file name will be the value in FileHeader.getFileName <br>
	 * <br>
	 * Throws an exception if file header could not be found for the given file
	 * name or if the destination path is invalid
	 * 
	...
	public void extractFile(String fileName, String destPath,
		...
		if (!Zip4jUtil.isStringNotNullAndNotEmpty(destPath)) {
			throw new ZipException(
					"destination string path is empty or null, cannot extract file");
		}
		...
		if (fileHeader == null) {
			throw new ZipException(
					"file header not found for given file name, cannot extract file");
		}
	 ...
	 * javase/1.5.0/docs/guide/security/jce/JCERefGuide.html#PBEEx
	 * 
	 * @param password
	 * @throws ZipException
	 */
		...
		for (int i = 0; i < zipModel.getCentralDirectory().getFileHeaders()
			...
			if (zipModel.getCentralDirectory().getFileHeaders().get(i) != null) {
				if (((FileHeader) zipModel.getCentralDirectory()
						.getFileHeaders().get(i)).isEncrypted()) {
					((FileHeader) zipModel.getCentralDirectory()
							.getFileHeaders().get(i)).setPassword(password);
				}
			}
		}
	}
	...
	public void removeFile(FileHeader fileHeader) throws ZipException {
		...
		archiveMaintainer.initProgressMonitorForRemoveOp(zipModel, fileHeader,
				progressMonitor);
		archiveMaintainer.removeZipFile(zipModel, fileHeader, progressMonitor,
				runInThread);
	}
	...
	public void mergeSplitFiles(File outputZipFile) throws ZipException {
		...
		ArchiveMaintainer archiveMaintainer = new ArchiveMaintainer();
		archiveMaintainer.initProgressMonitorForMergeOp(zipModel,
				progressMonitor);
		archiveMaintainer.mergeSplitZipFiles(zipModel, outputZipFile,
				progressMonitor, runInThread);
	}
	...
	public void setComment(String comment) throws ZipException {
		...
		if (zipModel.getEndCentralDirRecord() == null) {
			throw new ZipException(
					"end of central directory is null, cannot set comment");
		}
	...
	public String getComment(String encoding) throws ZipException {
		...
		if (this.zipModel.getEndCentralDirRecord() == null) {
			throw new ZipException(
					"end of central directory record is null, cannot read comment");
		}
		...
		try {
			return new String(this.zipModel.getEndCentralDirRecord()
					.getCommentBytes(), encoding);
